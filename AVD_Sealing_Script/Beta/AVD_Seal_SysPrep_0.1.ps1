...
function Recursive-SysPrep-AppX-Tidy {
    ...
    "$(Get-Date -Format HH:mm:ss:) Recursive-SysPrep-AppX-Tidy Called" | Out-File $logFile -Append

    Clear-SysPrepLogs
    Run-SysPrep
    Write-Host "Sleep for 10 seconds to allow SysPrep to start and error if problem AppX packages are present" -ForegroundColor DarkYellow
...
function Run-SysPrep {
    ...
    "$(Get-Date -Format HH:mm:ss:) Run-SysPrep Called" | Out-File $logFile -Append
...
function Remove-AppXPackages {
    ...
    "$(Get-Date -Format HH:mm:ss:) Remove-AppXPackages Called" | Out-File $logFile -Append
...
#           Output to Log file          #
#########################################
$logDir = "C:\_BUILD\SYSPREP_Test"
$scriptName = $MyInvocation.MyCommand.Name
$logFile = Join-Path -Path $logDir -ChildPath "$scriptName.buildlog"
...
"$(Get-Date -Format HH:mm:ss:) START OF LOG" | Out-File $logFile -Append
...
# Log the start of the script
"$(Get-Date -Format HH:mm:ss:) END OF SCRIPT" | Out-File $logFile -Append
